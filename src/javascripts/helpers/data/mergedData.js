  ...
  cowData.getCows().then((cowResponse) => {
    farmerData.getAllFarmers().then((farmerResponse) => {
      ...
      cowResponse.forEach((cow) => {
        const farmerObject = farmerResponse.find(
          (farmer) => farmer.uid === cow.farmerUid
        );
...
const getSingleFarmerView = (farmerUid) => new Promise((resolve, reject) => {
  farmerData.getSingleFarmer(farmerUid).then((farmerResponse) => {
    cowData.getFarmerCows(farmerResponse.uid).then((cowResponse) => {
